...
class simple_decoration_node_t : public wf::scene::node_t,
		...
		if (auto view = _view.lock()) {
			...
			if ((title_texture.tex.width != target_width) ||
				...
				auto surface = theme.render_text(
					view->get_title(), target_width,
					target_height);
				cairo_surface_upload_to_texture(
					surface, title_texture.tex);
			 ...
			 } }
	{
		...
		if (view->parent) {
			theme.set_buttons(wb::decor::button_type_t(
				wb::decor::BUTTON_TOGGLE_MAXIMIZE |
				wb::decor::BUTTON_CLOSE));
		} else {
			theme.set_buttons(wb::decor::button_type_t(
				wb::decor::BUTTON_MINIMIZE |
				wb::decor::BUTTON_TOGGLE_MAXIMIZE |
				wb::decor::BUTTON_CLOSE));
		}
				...
				wf::point_t origin, const wlr_box &scissor)
	{
		...
		for (auto item : renderables) {
			if (item->get_type() ==
			    wb::decor::DECORATION_AREA_TITLE) {
	...
	void handle_pointer_button(const wlr_pointer_button_event &ev) override
	{
		...
		handle_action(layout.handle_press_event(ev.state ==
							WLR_BUTTON_PRESSED));
	}

	void
	handle_action(wb::decor::decoration_layout_t::action_response_t action)
	{
		if (auto view = _view.lock()) {
			switch (action.action) {
			case wb::decor::DECORATION_ACTION_MOVE:
				return wf::get_core().default_wm->move_request(
					view);

			case wb::decor::DECORATION_ACTION_RESIZE:
				return wf::get_core().default_wm->resize_request(
					view, action.edges);

			case wb::decor::DECORATION_ACTION_CLOSE:
				return view->close();

			case wb::decor::DECORATION_ACTION_TOGGLE_MAXIMIZE:
				if (view->pending_tiled_edges()) {
					return wf::get_core()
						.default_wm->tile_request(view,
									  0);
				} else {
					return wf::get_core()
						.default_wm->tile_request(
							view,
							wf::TILED_EDGES_ALL);
				}
			...
			case wb::decor::DECORATION_ACTION_MINIMIZE:
				return wf::get_core()
					.default_wm->minimize_request(view,
								      true);
				break;
