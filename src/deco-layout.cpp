	...
	const decoration_theme_t &theme)
{
	...
	this->button = std::make_unique<button_t>(
		theme, std::bind(damage_callback, g));
}
...
wf::geometry_t decoration_layout_t::create_buttons(int width, int)
{
	...
	while (stream >> button_name) {
		if ((button_name == "minimize") &&
		    (theme.button_flags & BUTTON_MINIMIZE)) {
		...
		if ((button_name == "close") &&
		    (theme.button_flags & BUTTON_CLOSE)) {
	...
	wf::geometry_t button_geometry = {
		width - border_size +
			button_padding, /* 1 more padding initially */
									  ...
									  int y)
{
	...
	if (previous_area == current_area) {
		...
		if (current_area &&
		    (current_area->get_type() == DECORATION_AREA_BUTTON)) {
...
decoration_layout_t::handle_press_event(bool pressed)
{
	...
	if (!pressed && double_click_at_release) {
		...
		if (begin_area &&
		    (begin_area->get_type() == DECORATION_AREA_BUTTON)) {
			begin_area->as_button().set_pressed(false);
			if (end_area && (begin_area == end_area)) {
				switch (begin_area->as_button()
						.get_button_type()) {
				case BUTTON_CLOSE:
				...
				case BUTTON_TOGGLE_MAXIMIZE:
					return {
						DECORATION_ACTION_TOGGLE_MAXIMIZE,
						0
					};
